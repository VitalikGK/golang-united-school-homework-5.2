...
func NewCache(key, value string, deadline, stings time.Time) Cache {
...
func (m MapCache) Get(key string) (string, bool) {
	...
	// }
	return k.key + k.value + k.stings.Format("Jan _2 15:04:05.000000"), ok
}
...
func (m MapCache) Put(key, value string) {
	...
	if ok {
		...
		k = NewCache(key, value, time.Now(), time.Date(1, time.January, 1, 00, 0, 0, 0, time.UTC))
	...
	for key, _ := range m {
		...
		if ok {
			if k.deadline != time.Date(1, time.January, 1, 00, 0, 0, 0, time.UTC) {
				listCache = append(listCache, k.key+", "+k.value+", "+k.stings.Format("Jan _2 15:04:05.000000")+", "+k.deadline.Format("15:04:05"))
			}
		}
